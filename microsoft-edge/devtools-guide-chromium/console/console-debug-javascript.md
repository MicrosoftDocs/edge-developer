...
This means our requested URL was not found.  
...
However, you may also display in this example that our code is sloppy.  You still try to access the table and display the information.  Our code should catch errors and report them in the **Console** instead of doing that.  If you navigate to the [Network Error Reported][GithubMicrosoftedgeDevtoolssamplesConsoleNetworkErrorReportedHtml] example you may display that this example tells the user that something went wrong and the **Console** displays the Network error and the error message our code reported.  
...
You have three functions that request each other in succession, `first()`, `second()` and `third()`.  Each send a `name` argument to the other.  In the `third()` function you check if the `name` argument exists and if it is not, you log an error that name isn't defined.  If name is defined you use the `assert()` method to check if the `name` argument is less than eight letters long.  You request the first method three times, with the following changes.  

*   No argument which triggers the `console.error()` in `third()`.  
*   `Console` as the argument which causes no error as `name` exists and is shorter than eight letters.  
*   `Microsoft Edge Canary` as the argument, which triggers to assertion to report an error \(as the argument is longer than eight letter\).  
...
Notice that the assertion needs to record a negative result, that is why you test for `x < 40` in the if case and `x >= 40` for the assertion.  

If you are not sure which function requests which you may use the `console.trace()` method to track which functions were named to get to the current one.  To display this in the **Console**, navigate to [create traces example][GithubMicrosoftedgeDevtoolssamplesConsoleTraces] .  
