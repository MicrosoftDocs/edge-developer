...
1.  Hover over different parts of the rendered demo webpage.  Each element in the webpage now displays with a multicolor overlay. This multicolor overlay can display nested regions inside of an element. For example, hover over the left margin of **Cats**.  DevTools highlights several rectangular portions of **Cats** with different colors, which is the visual outcome of the CSS flexbox definitions on your webpage.
