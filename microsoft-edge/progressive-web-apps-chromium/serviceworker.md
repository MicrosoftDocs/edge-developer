...
For an example of what you can do with the `fetch` event, see [Use the caching to improve performance of PWAs](offline.md#use-the-caching-to-improve-performance-of-PWAs) in the article "Offline and network connectivity support in Progressive Web Apps".

<!-- show that chunk in present article, to show what its doing within the fetch, eg using fetch in sw's, ... the interlock is fine -->

Also see [Step 3 - Add a Service Worker](get-started.md#step-3---add-a-service-worker) in the article "Get started with Progressive Web Apps".  That section uses the **Cache-first network** service worker recipe from PWA Builder.  However, that approach is less useful than it previously was, because it uses workbox(todo: clarify/confirm) and therefore needs to be simplified and updated.
