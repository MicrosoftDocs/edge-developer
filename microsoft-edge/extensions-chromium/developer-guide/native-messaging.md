...
> [!NOTE] 
> If you have extensions both on the Microsoft Edge Add-ons and Chrome Webstore, you need to add the extension Ids corresponding to both the stores in the `allowed_origins` of the host manifest file because only the host manifest corresponding to the first registry location found will be read.
