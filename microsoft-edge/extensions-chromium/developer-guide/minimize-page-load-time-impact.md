---
title: Minimize your extension's impact on page load time
...
ms.date: 09/18/2024
---
# Minimize your extension's impact on page load time
...
1. To start recording a performance profile, click **Record** (![Record icon](./minimize-page-load-time-impact-images/record-icon.png)).

1. Reload the page to capture the profiling data that corresponds to the page load time and then, once the page has finished loading, click **Stop** (![Stop icon](./minimize-page-load-time-impact-images/stop-icon.png)) to end the recording. DevTools displays the recorded performance profile.
...
The Edge tracing tool, available at the `edge://tracing` URL, is a powerful tool that can provide detailed analysis of your extension's performance. In this section, you'll learn how to use the Edge tracing tool to understand the impact of your extension on the page load time. To learn more about the Edge tracing tool, which is based on the **Perfetto** tool, see [Perfetto UI](https://perfetto.dev/docs/visualization/perfetto-ui).

1. To open the Edge tracing tool, open a new tab or window, and go to `edge://tracing`. The tracing UI opens.

1. To start a new trace, in the top left corner of the tool, click **Record**. The **Record a new trace** dialog box appears.

1. In the **Record a new trace** dialog box, select the **Manually select settings** option. The list of categories appears.

1. To capture detailed information about your extension's content script compilation and execution, select the following categories:
...
1. Click **Record**. The Edge tracing tool starts recording the trace.
...
1. Open the tab where the Edge tracing tool is running, and then click **Stop**. The new trace information appears in the tool.

Traces recorded by the Edge tracing tool provide a lot of information about the browser. To filter the information to only what's relevant to the webpage that your extension impacted, use the following steps:

1. Press **Shift+Esc** to open the **Browser Task Manager** dialog box.

1. In the **Browser Task Manager** dialog box, search for the tab that corresponds to the webpage that your extension affected, and note the number in the **Process ID** column. Close the dialog box.
...
1. In the top right corner of the Edge tracing tool, click the search field, type **ScriptInjection::InjectJS**, and then press **Enter** until an event that corresponds to your extension gets highlighted in the bottom panel.

   The bottom panel displays the start time and total duration of the event.

   ![The Edge tracing tool showing a scrip injection event](./minimize-page-load-time-impact-images/tracing-event-selected.png)
...
* **v8.compile**: this event shows the compilation time of your content script.
* **v8.run**: this event indicates the running time of the compiled script.
...
* Lazy loading is the process of loading is the process of loading your code only when it's needed, based on the user actions, the page content, or the extension logic.
...
* Dynamic imports are only loaded on demand, for example when the user interacts with the webpage or with the extension's UI:
...
To reduce the amount of code that is loaded and that runs on each webpage, and to save memory and CPU resources, only load your content scripts as needed, instead of on every page load.
...
For example, you can use the `chrome.scripting` API to load your content script only after the user has interacted with the webpage or extension UI, such as when clicking on an extension button, or a part of the webpage.

If you use the `chrome.scripting` API when the user interacts with the webpage, make sure to carefully consider if you need to repeatedly load your content script every time the interaction happens. Loading content scripts too frequently can cause user experience issues or errors.
...
Long running synchronous tasks are those that take a long time to complete, preventing the browser from running other webpage code while they are running. This can include complex calculations, loops, or string manipulations.

Use asynchronous or non-blocking code, such as the Fetch API, JavaScript Promises, or Web Workers where possible. Asynchronous or non-blocking code allow the execution of other code while waiting for the completion of a task, without blocking browser process that runs the webpage.
