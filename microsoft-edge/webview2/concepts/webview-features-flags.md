...
| `edge-webview-unique-window-class` | Makes WebView2 register module-specific window classes by appending a suffix that's derived from the client module's file name, without a file path or file name extension.  For example, for the file name `EmbeddedBrowserWebView.dll`, the suffix `EmbeddedBrowserWebView` is appended to `Chrome_WidgetWin_0`, forming `Chrome_WidgetWin_0_EmbeddedBrowserWebView`.  Enable this flag, to avoid a window class collision crash that can occur when WebView2 and Chromium Embedded Framework (CEF) coexist in the same process while running different Chromium versions. |
