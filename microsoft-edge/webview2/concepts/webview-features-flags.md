...
| `edge-webview-unique-window-class` | Makes WebView2 register module-specific window classes by appending a suffix that's derived from the client module's file name, without path or extension.  For example, for the file name `EmbeddedBrowserWebView.dll`, `Chrome_WidgetWin_0` adds the suffix `EmbeddedBrowserWebView`, forming `Chrome_WidgetWin_0_EmbeddedBrowserWebView`.  Enable this flag, to avoid a window class collision crash that can occur when WebView2 and Chromium Embedded Framework (CEF) coexist in the same process while running different Chromium versions. |
