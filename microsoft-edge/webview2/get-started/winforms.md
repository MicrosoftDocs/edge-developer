...
1. In the **Toolbox**, click **WebView2 Windows Forms Control** to expand the options.
   ...
   > [!NOTE]
   > If you're using Visual Studio 2017, by default **WebView2** isn't displayed in the **Toolbox**.  To enable **WebView2** to be displayed in the **Toolbox**, select **Tools** > **Options** > **General** > and set the **Automatically Populate Toolbox** setting to `True`.
...
1. Resize the text box as shown.
   ...
   :::image type="content" source="./media/winforms-designer-txtbtn.png" alt-text="WinForms designer text box and button." lightbox="./media/winforms-designer-txtbtn.png":::
...
1. In the **Windows Forms Designer**, double-click on the `Go!` button to create the `goButton_Click` method in the `Form1.cs` file.  Paste the following snippet inside that method:
