...
If the host changes the `ResultFilePath` of the downloaded file, the file will only be downloaded if the app has access to that file path by default. If you would like to use a file location that the app does not have access to by default, you must set the corresponding capability. See [UWP Capabilities](https://learn.microsoft.com/windows/uwp/packaging/app-capability-declarations).
