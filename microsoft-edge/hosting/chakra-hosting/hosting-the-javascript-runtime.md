...
## Debugging and Profiling  
 ...
 Starting in Windows 10, the Chakra JavaScript Engine supports a legacy engine and Edge engine, and you can target either in JsRT (see [Targeting Edge vs. Legacy Engines](../chakra-hosting/targeting-edge-vs-legacy-engines-in-jsrt-apis.md) for details). Debugging a script in Visual Studio works differently between the legacy and the Edge engine. With the legacy engine, the host needs to provide an [IDebugApplication Interface](https://docs.microsoft.com/en-us/scripting/winscript/reference/idebugapplication-interface) pointer, which can be obtained from an [IProcessDebugManager Interface](https://docs.microsoft.com/en-us/scripting/winscript/reference/iprocessdebugmanager-interface) instance. With the Edge engine, `IDebugApplication` is deprecated, and the Chakra engine enables native and script debugging capabilities through the Visual Studio debugger without requiring an implementation of `IDebugApplication` from the user.  
