...
Bing Chat is an AI-enhanced web search experience that supports you by summarizing web search results and providing a chat experience. You can also generate creative content, such as poems, jokes, and letters. The new AI-enhanced Bing runs on a variety of advanced technologies from Microsoft and OpenAI, including GPT-4, which is a large language model (LLM) from OpenAI.

When you enter text in a prompt in Bing Chat, the following information is sent as input to the large language model:
* The prompt.
...
The response that's generated by the large language model:
* Uses the page context, your prompt, and recent conversation history to contextualize the request.
* Uses the meta prompt to align responses with Microsoft AI Principles and user expectations.
* Uses the search results to ground responses in existing, high-ranking content from the web.

Responses are presented to you in several different formats, such as traditional links to web content, AI-generated summarizations, and chat responses.  Summarizations and chat responses that rely on web search results include references and a **Learn more** section below the responses, providing links to search results that were used to ground the response.

The page context consists of both the page URL and the text on the webpage (or in a local document open in the main pane).  This data is only used after explicit consent from you, and is only used when the **Chat** pane is open.  You can close the **Chat** pane if you want to temporarily stop the data sharing, or you can permanently update your preferences in the Microsoft Edge settings.

![Bing Chat pane](./index-images/bing-chat.png)

To change settings for Bing Chat, go to `edge://settings/sidebar`, and then select **App and notification settings** > **Bing Chat**.

Conversations on public pages are automatically saved for future reference (you can opt out of auto-saving).  To protect your privacy, if a conversation moves to a private page, a local document, or a page behind a paywall, the conversation isn't saved.

Bing Chat can also perform scoped Microsoft Edge actions when you enter commands.  For example, sending "Help me organize my tabs into groups" in Bing Chat invokes the **Group similar tabs** feature.  This enables you to discover advanced Microsoft Edge settings and take actions by typing commands in chat in natural language.  Microsoft Edge doesn't store or use any information that's passed along to Bing Chat to complete the action.
...
<!-- ====================================================================== -->
## Bing Chat in Edge sidebar (draft 2)

With Bing Chat in the Edge sidebar you can ask complex questions, find comprehensive answers, get summarized information, and find inspiration to build upon, in a side-by-side view, with no need to flip between tabs.  To turn on Bing Chat in Edge sidebar, simply click on the Bing icon on the upper right of your Edge toolbar.


<!-- --------------------------------------------------------------------- -->
#### What data and consent does Microsoft Edge use to generate a response on Bing Chat in Edge sidebar?

Microsoft Edge determines what data to send to Bing Chat based on the user's **query** and their consent to share data with Microsoft.

For questions that do not need **browsing context**,  (e.g. “Help me plan a trip to Canon beach”) Microsoft Edge shares the user's query and previous conversation history to help Bing Chat answer their query effectively.
...
![Contextual consent](./index-images/consent-for-contextual-browsing.png)

The user can modify this permission anytime by using the Microsoft Edge setting **Allow Microsoft to Answer Page Content** in the **Settings** menu:

<!-- todo: UI not found, screenshot?  improve UI path wording -->
`edge://settings/sidebar/appSettings?hubApp=2354565a-f412-4654-b89c-f92eaa9dbd20`


<!-- ---------------------------------------------------------------------- -->
#### How long does Microsoft retain conversation history?

1. User **conversation history** (a history of all queries and responses from Bing Chat) is stored for 90 days. This is agnostic of using Bing Chat in the Edge sidebar or by visiting www.bing.com/chat

1. **Page context** from public **pages on the Bing index** – Bing Chat uses publicly available information from the Bing search engine to summarize pages that are available on the Bing search index. Microsoft Edge does not need to share this context with Bing Chat to answer questions from public pages.

1. **Page context** from **non-Bing index** pages – information used to summarize a non-Bing index page is deleted after 6 hours and it is only used to answer the query.

1. Users can turn off saving conversations in Bing Chat, by going to **Settings** on Bing.com.  From here, the user can delete the entire conversation history as well as specific conversations.

![Recent conversations](./index-images/bing-chat-recent.png)


<!-- ---------------------------------------------------------------------- -->
### Our commitment to responsible AI

Like any other transformational technology, Bing Chat has numerous protective measures in place that are constantly evolving.  For details, see [The new Bing: Our approach to Responsible AI](https://blogs.microsoft.com/wp-content/uploads/prod/sites/5/2023/04/RAI-for-the-new-Bing-April-2023.pdf).

To learn more about how Bing Chat for Enterprises protects user data and business data, see [Privacy and protections](/bing-chat-enterprise/privacy-and-protections).
...
| _Bing index page_ | Any website URL that is publicly available on the internet for Bing to scrape and can be reached via a URL that starts with `http://` or `https://`. |
| _Non-Bing index page_ | Any webpage or local file that is not available publicly for Bing to scrape but is open on Microsoft Edge. |
...
| _Browsing context_ | Browsing context is comprised of page URL, web page content, or<!--todo: contradicts "comprised"--> documents that are open in the active tab of the Microsoft Edge browser, such as webpages, PDFs, Word documents, or files.  This data helps Bing Chat generate a contextually relevant response.  For example, the user can ask **Summarize this page** while on a Wikipedia<!--todo: probably must use a Microsoft-owned page instead--> page about a topic; Bing Chat understands that the page the user is referring to is the Wikipedia<!--todo--> page, and summarizes that page in the response. |
