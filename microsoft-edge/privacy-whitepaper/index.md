...
In the Bing side pane, Compose uses AI to write for you starting from a short prompt and "tone", "format", and "length" selections. Text generated by Compose can be copied or inserted into the text page directly. To access Compose, click on the blue Bing icon in the right hand pane > Compose (requires Bing waitlist approval). Text and selections inputed by the user is sent to an internal Microsoft Large Language Model (LLM).

![Compose](./media/compose.png)

The request is sent over a secure HTTPS connection with encryption and contains the input prompt, formatting selections, along with IDs associated with the user's Edge profile and device, to reference for subsequent requests. User input is not stored at all. 
