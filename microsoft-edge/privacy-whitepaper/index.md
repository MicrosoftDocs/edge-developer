...
The request is sent over a secure HTTPS connection. The request contains the input prompt, formatting selections, and IDs that are associated with the user's Microsoft Edge profile and device, to reference for subsequent requests. User input is not stored at all. 
