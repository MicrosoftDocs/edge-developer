...
Bing Chat is an AI-enhanced web search experience. It supports users by summarizing web search results and providing a chat experience. Users can also generate creative content, such as poems, jokes, and letters. The new AI-enhanced Bing runs on a variety of advanced technologies from Microsoft and OpenAI, including GPT-4, a large language model (LLM) from OpenAI.

When a user enters text in a prompt in Bing, the following information is sent as input to the large language model:
* The prompt.
...
* Uses the page context, user's prompt, and recent conversation history to contextualize the request.
...
The page context consists of both the page URL and the text on the webpage (or in a local document open in the main pane). This data is only used after explicit consent from the user, and is only used when the **Chat** pane is open. The user can close the **Chat** pane if they want to temporarily stop the data sharing, or can permanently update their preferences in the Microsoft Edge settings.

![Bing Chat pane](./media/bingchat.png)

Conversations on public pages are automatically saved for future reference (you can opt out of auto-saving). If a conversation moves to a private page, local document, or a page behind a paywall, the conversation isn't saved, to protect users' privacy. 

Bing chat can also perform scoped Microsoft Edge actions on user command. For example, sending "Help me organize my tabs into groups" on Bing chat will invoke the existing **Group similar tabs** feature. This enables users to discover advanced Microsoft Edge settings and take actions by just typing commands in chat in natural language. Microsoft Edge doesn't store or use any information that's passed along to Bing chat to complete the action.
...
In the Bing side pane, Compose uses AI to write for you, starting from a short prompt and **Tone**, **Format**, and **Length** selections. Text that's generated by Compose can be copied or inserted into the webpage directly. To access Compose, click the blue Bing icon in the right-hand pane > **Compose**. Text and selections that you enter are sent to an internal Microsoft Large Language Model (LLM).
...
![Rewards Dashboard UI](./media/rewardsdashboard.png)

![Rewards Sidepane UI](./media/rewardssidepane.png)
...
![Rewards Settings UI](./media/rewardssettings.png)
...
![Auto Tab Group UI](./media/tabgrouping2.gif)
...
![Tab Group settings UI](./media/tabgroupsettings.png)

When the Tab Organization service is turned off, Tab groups can still be created, but will be given a generic name such as **Group 1**, and you can rename the Tab group to be more descriptive.  The option to automatically Group Similar Tabs within the Tab Action Menu will be turned off.
