...
Bing chat is an AI-enhanced web search experience. It supports users by summarizing web search results and providing a chat experience. Users can also generate creative content, such as poems, jokes, and letters. The new AI-enhanced Bing runs on a variety of advanced technologies from Microsoft and OpenAI, including GPT-4, a cutting-edge large language model (LLM) from OpenAI. 

When a user enters a prompt in the new Bing, the prompt, recent conversation history, the meta prompt, page context, and top search results are sent as inputs to the LLM. The model generates a response using the page context, user’s prompt, and recent conversation history to contextualize the request, the meta prompt to align responses with Microsoft AI Principles and user expectations, and the search results to ground responses in existing, high-ranking content from the web.  
...
Page context comprises of both page URL and the text on the webpage or local document open in the main pane. This data is only used after explicit consent from the user and only when the chat pane is open. User can close the chat pane if they want to stop the data sharing temporarily or permanently update their preferences in the Edge settings.

![BingChatPane](./index-images/bingchat.png)

Conversations on public pages are automatically saved for future reference (You can opt out of auto-saving). If conversation moves to a private page / local documents / or pages behind paywalls, conversation will not be saved to protect users’ privacy. 
Bing chat can also perform scoped Edge actions on user command. For example, sending “Help me organize my tabs into groups” on Bing chat will invoke the existing Group similar tabs feature. This enables users to discover advanced Edge settings and take actions by just typing commands in chat in natural language. Edge does not store or use any information passed along to Bing chat to complete the action.

Like any other transformational technology, Bing chat has numerous protective measures in place that are constantly evolving. For details, please refer to [this document](https://blogs.microsoft.com/wp-content/uploads/prod/sites/5/2023/04/RAI-for-the-new-Bing-April-2023.pdf).
...
In the Bing side pane, Compose uses AI to write for you, starting from a short prompt and **Tone**, **Format**, and **Length** selections. Text that's generated by Compose can be copied or inserted into the web page directly. To access Compose, click the blue Bing icon in the right hand pane > **Compose**. Text and selections that you enter are sent to an internal Microsoft Large Language Model (LLM).

![Compose UI](./index-images/compose.png)

Requests for Compose are sent over a secure HTTPS connection. Each request contains the input prompt, formatting selections, and IDs that are associated with the user's Microsoft Edge profile and device, to reference for subsequent requests. 
...
Microsoft Rewards (the "Program") enables you to earn redeemable points for activities such as qualified searches, acquisitions, and other offers from Microsoft. These points can then be redeemed for gift cards or non-profit donations or automatic contributions to subscriptions. Microsoft Edge has built-in support for all existing avenues for earning points and also introduces new and exclusive means to earn more than ever. You can easily track your points, stay up to date with limited period offers to get the most out of the program.
For non-members, Microsoft Edge presents personalized joining offers based on the page you’re on. Once you engage and choose to enroll, Edge uses your Microsoft account identity to sign you up and start the rewards service that automatically adds points to your account for searching, playing and shopping on Edge. Edge does not send your personal information or browsing data to any third-party services. 
...
![RewardsDashboardUI](./index-images/rewardsdashboard.png)

![RewardsSidepaneUI](./index-images/rewardssidepane.png)
...
![RewardsSettingsUI](./index-images/rewardssettings.png)

You can also completely opt out of the Rewards program. Please see the the details below: 
Note:
...
Microsoft Edge can offer helpful suggestions on how to organize tabs to save you time and keep you focused on the web content you care about. These suggestions augment the built in Tab Grouping feature built into Microsoft Edge.  

When two or more tabs are grouped together, Microsoft Edge sends information about the tabs including the Title and URL to the service over HTTPS.  This information is used to generate a relevant name for the Tab Group.

![TabGroupUI](./index-images/tabgrouping1.gif)

Additionally, you can choose to have Microsoft Edge auto-group all of your tabs using the “Group Similar Tabs” feature from the Tab Action Menu.  In addition to the Title and URL, Microsoft Edge will also send information on which tabs opened another, and a timestamp on when a tab was opened.  This additional data allows the service to more accurately suggest Tab Groups to organize your tabs. 

![AutoTabGroupUI](./index-images/tabgrouping2.gif)

The Microsoft Edge Tab organization service is enabled by default and can be disabled in the Privacy settings: edge://settings/privacy. 

![TabGroupSettingsUI](./index-images/tabgroupsettings.png)

When disabled, Tab Groups can still be created, but will be given a generic name (eg: “Group 1”) when created and will likely need you to rename for context.  The option to automatically Group Similar Tabs within the Tab Action Menu will be disabled.
