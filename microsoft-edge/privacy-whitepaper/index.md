...
To access the Compose tool, in the upper right of Microsoft Edge, click the **Copilot** (![Copilot icon](./index-images/copilot-icon.png)) button, and then select **Compose**.  Or, press **Ctrl+Shift+.**.  Text that you select or enter is sent to an internal Microsoft service that runs a Large Language Model (LLM).
