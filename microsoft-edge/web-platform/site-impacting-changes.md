...
| Show a subtle prompt for notification permission requests | Canary experimentation starting in M83 | When sites request to send users notifications via the `Notifications` or `Push` APIs, Edge shows a subtle request icon in the address bar, instead of the flyout permission prompt. This experiment applies to all users on all sites which request notifications permissions, unless the user opts out via `edge://settings/content/notifications`. In the future, we may explore showing the full flyout prompt in certain situations based on user behaviors and other signals. | 
