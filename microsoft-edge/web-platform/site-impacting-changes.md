...
| Display subtle prompt for notification permissions requests | Edge v84 |  | Microsoft Edge enables by default the Quiet Notifications feature. This UI (user interface) feature displays a subtle request icon in the address bar for site notification permissions requested via the `Notifications` or `Push` API, replacing the full/standard permission flyout prompt UI. Users may opt out of this UI treatment in `edge://settings/content/notifications`. In the future, the Microsoft Edge team may explore re-enabling the full flyout notification prompt in some scenarios.  |  
