...
| --- | --- | --- | --- |
| Insecure downloads over HTTP | Future release (TBD) | | Users that download potentially dangerous content on HTTP sites will receive a UI warning, such as "Insecure download blocked."  The user will still have an option to proceed and download the item.  Admins can use the `InsecureContentAllowedForUrls` policy to specify HTTP sites where the warning will be suppressed.  Admins can use the `InsecureDownloadWarnings` feature flag to test the impact of this upcoming feature. |
