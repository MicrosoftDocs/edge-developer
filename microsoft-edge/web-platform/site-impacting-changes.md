...
| Three Digit User-Agent String Version | v100 | | As of version 100, the browser will begin sending a three-digit version in the User-Agent header (`Edg/100`). This may confuse scripts or server-side analytics that use buggy parsers to determine the User-Agent string version number. To emulate this condition before version 100, use the command line argument `--enable-features=ForceMajorVersion100InUserAgent`|
